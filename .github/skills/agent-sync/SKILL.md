...
- Protected root markdown files (`AGENTS.md`, `README.md`, `AGENTS.local.md`, `README.local.md`) are blocked from any sync by default and require explicit user approval plus script flag `-AllowRootMdSync`. This includes the marker-based injection of the universal governance section from `.github/skills/shared/agents-core-rules.md` into an existing target `AGENTS.md` (using `<!-- BEGIN AGENT SYNC --> … <!-- END AGENT SYNC -->` markers): it also requires `-AllowRootMdSync`, and a plain `agent-sync` run does not touch `AGENTS.md` at all. What `-AllowRootMdSync` changes for `AGENTS.md` specifically is that only the marked section is written — client content outside the markers is never modified — whereas the other protected files are full-file replaced.
...
8. Update target root `.gitignore` with exact synced file paths, excluding paths containing `agent-sync` or `sync-agent-assets`. This includes every discovered OpenCode agent, command, `opencode.jsonc`, and parity-map file.
...
- Never write protected root markdown files (`AGENTS.md`, `README.md`, `AGENTS.local.md`, `README.local.md`) unless user approval and `-AllowRootMdSync` are both present. This includes the marker-based injection of the governance section (from `.github/skills/shared/agents-core-rules.md`) into an existing `AGENTS.md` — it is gated behind `-AllowRootMdSync` exactly like every other protected-root-md operation, so a plain `agent-sync` run leaves `AGENTS.md` (markers included) untouched. The only thing that sets `AGENTS.md` apart under `-AllowRootMdSync` is that just the marked section is rewritten rather than the whole file.
