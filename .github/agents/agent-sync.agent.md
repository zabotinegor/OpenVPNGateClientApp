...
## Mission
Synchronize agent, skill, and tool assets from the source repository `https://github.com/zabotinegor/CopilotTools` (branch `main`) into the current repository, then report exactly what changed.
...
7. Produce a detailed change report grouped by Added, Changed, and Deleted, with verification status.
