...
| Doc | Covers |
|---|---|
| [guides/how-to.md](guides/how-to.md) | Reusable techniques: MockWebServer tests, hardprobe lifecycle, on-device SSE verification, local mock backend, prefs-migration checks, Gradle test/compile-worker OOM recovery (test-executor and Kotlin-compile-worker forms, `FROM-CACHE` trap), detecting a vacuous regression test with targeted single-guard mutation testing |
