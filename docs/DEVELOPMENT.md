...
Technical reference for building, configuring, and understanding this repository. For a
user-facing introduction to the app itself, see the [root README](../README.md).
...
## Tech Stack
- Kotlin, Android SDK 24+ (app modules target/compile 36; the engine submodule needs SDK 37 — see [reference/build-config.md](reference/build-config.md))
- ViewBinding
...
- Country **resolution** is code-first: the stable code is stored per server, and the persisted country display name is rewritten on language change rather than matched against. See [features/server-selection.md](features/server-selection.md).
...
the root copy is ignored — see [reference/build-config.md](reference/build-config.md).
...
**[INDEX.md](INDEX.md) is the catalog for all technical documentation** in this repository
...
- Sustained unhealthy connected state triggers auto-recovery with a failure threshold and cooldown; success resets watchdog counters and reconnecting hints. At most three recoveries until traffic actually flows — the count survives the watchdog's own reconnects — then a fail-safe disconnect. See [features/connection-recovery.md](features/connection-recovery.md).
...
- [INDEX.md](INDEX.md): the knowledge-base catalog — start here for anything behavioural.
