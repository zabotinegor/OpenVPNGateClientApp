...
8. After ANY wakeup, auto-resume, or session restore: re-read `.sdlc/status.json` (steps, substeps, `lastUpdatedUtc`) and re-derive the entry step from the flow's resume table before acting — never resume from wakeup reason text, conversation memory, or `checkpoint.currentStep` alone; another session (possibly another account) may have advanced the flow while this one slept. If the flow's `lastUpdatedUtc` is newer than your checkpoint and within the last ~10 minutes, another session is likely active — schedule a fresh wakeup (~15 min) or ask the user instead of double-running steps.
