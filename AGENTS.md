...
- The related backend/API codebase lives outside this workspace at `local path from AGENTS.local.md`.
...
- If a task changes API contracts for updates, releases, version metadata, or server-list payloads, inspect the backend implementation in `local path from AGENTS.local.md` and keep client/server formats aligned.
